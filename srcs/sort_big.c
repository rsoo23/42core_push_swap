...
/*   Updated: 2023/05/25 00:13:59 by rsoo             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	exp_half_sort_rem(t_dlist **stack_a, t_dlist **stack_b, t_info *info)
{
	...
	original_size = info->size_b;
	info->part_size = info->size_b / 2;
	info->pivot = info->size_b - info->part_size / 2;
	info->lower_lim = info->size_b - info->part_size;
	// printf("sizeb: %d, lowerlim: %d, partsize:%d\n", info->size_b, info->lower_lim, info->part_size);
	while (info->size_b > info->pivot)
	{
		tail_b = ft_dlstlast(*stack_b);
		if (tail_b->index > info->pivot)
		{
			push('a', stack_a, stack_b);
			info->size_b--;
		}
	...
	while (info->size_b < original_size)
	{
